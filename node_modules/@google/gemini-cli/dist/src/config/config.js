...
export async function loadCliConfig(settings, extensions, sessionId, argv) {
    ...
    const ideModeFeature = argv.ideModeFeature ?? settings.ideModeFeature ?? false;
    ...
    const { memoryContent, fileCount } = await loadHierarchicalGeminiMemory(process.cwd(), settings.loadMemoryFromIncludeDirectories ? includeDirectories : [], debugMode, fileService, settings, extensionContextFilePaths, memoryImportFormat, fileFiltering);
    let mcpServers = mergeMcpServers(settings, activeExtensions);
    const question = argv.promptInteractive || argv.prompt || '';
    const approvalMode = argv.yolo || false ? ApprovalMode.YOLO : ApprovalMode.DEFAULT;
    ...
    const extraExcludes = !interactive && approvalMode !== ApprovalMode.YOLO
        ...
        : undefined;
    const excludeTools = mergeExcludeTools(settings, activeExtensions, extraExcludes);

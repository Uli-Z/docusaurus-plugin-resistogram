...
import { getDialogSettingKeys, getSettingValue, setPendingSettingValue, getDisplayValue, hasRestartRequiredSettings, saveModifiedSettings, getSettingDefinition, isDefaultValue, requiresRestart, getRestartRequiredFromModified, getDefaultValue, } from '../../utils/settingsUtils.js';
...
export function SettingsDialog({ settings, onSelect, onRestartRequest, }) {
            ...
            setPendingSettings((prevPending) => {
                ...
                modifiedSettings.forEach((key) => {
                    ...
                    if (storedValue !== undefined) {
                        updatedPending = setPendingSettingValue(key, storedValue, updatedPending);
                    }
                });
            ...
            return {
                ...
                toggle: () => {
                    ...
                    if (!requiresRestart(key)) {
                        ...
                        const immediateSettingsObject = setPendingSettingValue(key, newValue, {});
                        ...
                        setPendingSettings((_prevPending) => {
                            let updatedPending = structuredClone(settings.forScope(selectedScope).settings);
                            currentModifiedSettings.forEach((modifiedKey) => {
                                if (modifiedKey !== key) {
                                    const modifiedValue = modifiedValues.get(modifiedKey);
                                    if (modifiedValue !== undefined) {
                                        updatedPending = setPendingSettingValue(modifiedKey, modifiedValue, updatedPending);
                                    }
                                }
                            });
                            return updatedPending;
            ...
            if (key.upArrow || input === 'k') {
                ...
                if (currentSetting) {
                    ...
                    if (!requiresRestart(currentSetting.value)) {
                        ...
                        const immediateSettingsObject = setPendingSettingValue(currentSetting.value, booleanDefaultValue, {});
                        saveModifiedSettings(immediateSettings, immediateSettingsObject, settings, selectedScope);
                    }
                }
            }
    ...
    return (_jsx(Box, { borderStyle: "round", borderColor: Colors.Gray, flexDirection: "row", padding: 1, width: "100%", height: "100%", children: _jsxs(Box, { flexDirection: "column", flexGrow: 1, children: [_jsx(Text, { bold: true, color: Colors.AccentBlue, children: "Settings" }), _jsx(Box, { height: 1 }), showScrollUp && _jsx(Text, { color: Colors.Gray, children: "\u25B2" }), visibleItems.map((item, idx) => {
                    ...
                    const displayValue = getDisplayValue(item.value, scopeSettings, mergedSettings, modifiedSettings, pendingSettings);

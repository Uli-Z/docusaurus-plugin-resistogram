        ...
        describe('install', () => {
            it('should call execSync with the correct command and return success', async () => {
                const execSyncSpy = vi
                    ...
                    .mockImplementation(() => '');
                const result = await installer.install();
                expect(execSyncSpy).toHaveBeenCalledWith('npx ovsx get google.gemini-cli-vscode-ide-companion', { stdio: 'pipe' });
                expect(result.success).toBe(true);
                expect(result.message).toContain('VS Code companion extension was installed successfully from OpenVSX');
            });
            it('should return a failure message on failed installation', async () => {
                ...
                const result = await installer.install();
                expect(result.success).toBe(false);
                expect(result.message).toContain('Failed to install VS Code companion extension from OpenVSX');
            });
        });
    });
});

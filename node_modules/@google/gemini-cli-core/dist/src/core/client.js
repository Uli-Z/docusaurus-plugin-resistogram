...
import { MalformedJsonResponseEvent, NextSpeakerCheckEvent, } from '../telemetry/types.js';
...
export class GeminiClient {
    ...
    async *sendMessageStream(request, signal, prompt_id, turns = this.MAX_TURNS, originalModel) {
        ...
        if (this.config.getIdeModeFeature() && this.config.getIdeMode()) {
            const { contextParts, newIdeContext } = this.getIdeContextParts(this.forceFullIdeContext || this.getHistory().length === 0);
    ...
    async tryCompressChat(prompt_id, force = false) {
        ...
        if (!force) {
            const threshold = contextPercentageThreshold ?? this.COMPRESSION_TOKEN_THRESHOLD;

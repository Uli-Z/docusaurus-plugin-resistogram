...
describe('WriteFileTool', () => {
    ...
    describe('validateToolParams', () => {
        ...
        it('should return error if path is a directory', () => {
            ...
            expect(tool.validateToolParams(params)).toMatch(`Path is a directory, not a file: ${dirAsFilePath}`);
        });
        it('should return error if the content is null', () => {
            ...
            expect(tool.validateToolParams(params)).toMatch(`params/content must be string`);
        });
    });
    describe('getDescription', () => {
        it('should return error if the file_path is empty', () => {
            ...
            expect(tool.getDescription(params)).toMatch(`Model did not provide valid parameters for write file tool, missing or empty "file_path"`);
        });
    });
    describe('_getCorrectedFileContent', () => {
        it('should call ensureCorrectFileContent for a new file', async () => {
            ...
            const result = await tool._getCorrectedFileContent(filePath, proposedContent, abortSignal);
            expect(mockEnsureCorrectFileContent).toHaveBeenCalledWith(proposedContent, mockGeminiClientInstance, abortSignal);
    ...
    describe('execute', () => {
        ...
        it('should return error if params are invalid (path outside root)', async () => {
            ...
            const result = await tool.execute(params, abortSignal);
            expect(result.llmContent).toContain('Could not write file due to invalid parameters:');
            expect(result.returnDisplay).toContain('File path must be within one of the workspace directories');

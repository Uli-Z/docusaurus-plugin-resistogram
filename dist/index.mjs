...
var loadCsv = (dir, file) => readFile(join(dir, file), "utf8").then(CSV);
...
var loadResistanceDataForSource = (source, dataDir) => {
...
// src/index.ts
function docusaurusPluginResistogram(ctx, opts = {}) {
  ...
  const dataPath = join2(siteDir, dataDir);
  const pluginDataDir = join2(
    generatedFilesDir,
  ...
  return {
    ...
    async contentLoaded({ actions }) {
      ...
      for (const source of sources) {
        ...
        (0, import_fs_extra.writeJsonSync)(join2(pluginDataDir, fileName), compressedData);
        resistanceDataFileNames.set(source.id, fileName);
      }
      const classToAbx = /* @__PURE__ */ new Map();
      ...
      const sharedData = {
        ...
        id2MainSyn: Object.fromEntries(
          new Map([...abx, ...org].map((r) => [r.amr_code, r.full_name_de]))
        ),
        id2ShortName: Object.fromEntries(
          new Map([...abx, ...org].map((r) => [r.amr_code, r.short_name_de]))
        ),
        classToAbx: Object.fromEntries(classToAbx),
        allAbxIds
        // Still needed for sorting/grouping on the client
      };
      (0, import_fs_extra.writeJsonSync)(join2(pluginDataDir, sharedDataFileName), sharedData);
      const globalData = {
        sources,
    ...
    async postBuild({ outDir }) {
      const destDir = join2(outDir, "assets", "json");
      (0, import_fs_extra.ensureDirSync)(destDir);
      (0, import_fs_extra.copySync)(pluginDataDir, destDir);
    },
    getThemePath() {
      return "./theme";
    },
...
export {
  docusaurusPluginResistogram as default
};
